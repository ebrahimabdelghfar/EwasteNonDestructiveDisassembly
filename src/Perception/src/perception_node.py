...
class PerceptionNode():
    def loadModel(location="/home/omar/Desktop/GP/EwasteNonDestructiveDisassembly/src/Perception/src/epoch_480.pt"): 
        #load model 

        model = attempt_load(location,"cpu")
        model = TracedModel(model,"cpu",640)



    def __init__(self) -> None:
        self.perceptionNode=rospy.init_node("Perception")
        self.perceptionPublisher=rospy.Publisher('ListOfScrews' ,String,queue_size=50)
    ...
    def pixelToSpace(self,predresults,debug=False):
        screwlist=[]
        for predresult in predresults:
            ...
            xyz_position = self.verts[centery][centerx]
            xyz_position = [float(xyz_position[0]),float(xyz_position[1]),float(xyz_position[2])]
            #print(xyz_position)
            screwlist.append(xyz_position)
            rc=cv2.rectangle(self.color_image,(int(predresult[0]),int(predresult[1])),(int(predresult[2]),int(predresult[3])),color,2)
        if debug:
